...
import type { LeftNavConfig } from './LeftNavOld.types';

const meta: Meta<typeof LeftNavOld> = {
  // Out of `Components`, like Sidebar before it: a deprecated thing sitting in
  // the same list as everything current is a thing somebody will pick.
  title: 'Deprecated/LeftNav Old',
  component: LeftNavOld,
  tags: ['autodocs'],
  parameters: {
    layout: 'fullscreen',
    status: {
      ...
      deprecation: {
        ...
        replacement: 'LeftNav',
        message:
          'The product navigation (workspace rail, then the Settings and Agent Fleet floors) replaced this settings-only arrangement on 3 September 2026. It stays for side-by-side comparison until that replacement is approved.',
      },
    },
    docs: {
      description: {
        component: [
          ...
          '',
          '`LeftNav` is now the product navigation ported from the merged console: the workspace',
          'rail with the account card and folder tree, and beneath it the Settings and Agent Fleet',
          'floors reached through the crumb strip. This component is the earlier settings-only',
          'arrangement (levels that open in place, an exit row, a section header). It is kept so the',
          'two can be reviewed side by side, and goes when the replacement is approved.',
          '',
          '**Do not start anything new on it.** Its parts (`LeftNavItem`, `LeftNavGroup`,',
          '`useLeftNavLevels`…) keep their names until the removal pull request.',
        ].join('\n'),
      },
    },
  },
 ...
 *
 * `LeftNavOld` is parts; `DataLeftNav` is what you reach for when the navigation is
