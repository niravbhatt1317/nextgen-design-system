...
export interface PersonCellOld2Props {
  name: string;
...
function PersonCellOld2({
  ...
  return (
    ...
    >
      ...
      {owner && (
        <span
          className="tbl-old2-owner mdt-inline-flex mdt-h-5 mdt-w-5 mdt-shrink-0 mdt-items-center mdt-justify-center mdt-rounded-full mdt-text-azure-60"
          role="img"
...
export interface ContactChipsOld2Props {
  email?: string | null;
  phone?: string | null;
...
function ContactChipsOld2({ email, phone, onCopy }: ContactChipsOld2Props) {
  if (!email && !phone) return <TableEmptyValueOld2 />;
  return (
    <TooltipProvider>
...
export interface TagListOld2Props {
  items: string[];
...
function TagListOld2({ items, max = 2 }: TagListOld2Props) {
  if (items.length === 0) return <TableEmptyValueOld2 />;
...
export { PersonCellOld2, ContactChipsOld2, TagListOld2, TableEmptyValueOld2 };
