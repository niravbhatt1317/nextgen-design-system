...
describe('useTableColumnsOld2', () => {
  ...
  it('remembers the layout under a storage key and reconciles new columns', () => {
    const first = renderHook(() => useTableColumnsOld2({ columns: COLS, storageKey: 'k' }));
    act(() => {
      first.result.current.hide('c');
      first.result.current.setWidth('a', 300);
      first.result.current.moveToEnd('a');
    });
    const more: TableColumnDefOld2<Row>[] = [...COLS, { key: 'd', label: 'D', cell: () => null }];
    const second = renderHook(() => useTableColumnsOld2({ columns: more, storageKey: 'k' }));
...
describe('useTableSelectionOld2', () => {
  it('toggles, extends a range with Shift, and reports the header state', () => {
    const { result } = renderHook(() => useTableSelectionOld2());
...
describe('useTableSortOld2', () => {
  ...
  it('cycles A to Z, Z to A, off, and sorts case-insensitively', () => {
    const { result } = renderHook(() => useTableSortOld2());
  ...
  it('starts from a sort it was given', () => {
    const { result } = renderHook(() => useTableSortOld2({ key: 'age', direction: 'asc' }));
  ...
  it('moves to a new column A to Z rather than continuing the cycle', () => {
    const { result } = renderHook(() => useTableSortOld2({ key: 'name', direction: 'desc' }));
    act(() => result.current.cycle('age'));
    expect(result.current.sort).toEqual({ key: 'age', direction: 'asc' });
  });

  it('clears', () => {
    const { result } = renderHook(() => useTableSortOld2({ key: 'name', direction: 'asc' }));
    act(() => result.current.clear());
    expect(result.current.sort).toBeNull();
  });
});
...
describe('useTableSortOld2, values that are not plain text', () => {
  interface Mixed {
    id: string;
    tags: string[];
    active: boolean;
  ...
  const col = (key: string): TableColumnDefOld2<Mixed> => ({ key, label: key, cell: () => null });
  ...
  const sorted = (key: string, from: Mixed[] = rows) => {
    const { result } = renderHook(() => useTableSortOld2({ key, direction: 'asc' }));
  ...
  it('sorts on a key with no column of its own', () => {
    const { result } = renderHook(() => useTableSortOld2({ key: 'id', direction: 'desc' }));
...
describe('useTablePagingOld2', () => {
  ...
  it('loads more in batches in the other mode', () => {
    const { result } = renderHook(() => useTablePagingOld2({ pageSize: 25, mode: 'loadMore' }));
    expect(result.current.slice(rows)).toHaveLength(25);
