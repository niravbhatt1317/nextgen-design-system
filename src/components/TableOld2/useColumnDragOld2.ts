...
export interface ColumnDragStateOld2 {
  key: string;
  label: string;
...
export interface UseColumnDragOld2Options {
...
export interface UseColumnDragOld2 {
  drag: ColumnDragStateOld2 | null;
 ...
 */
export function useColumnDragOld2({
  viewportRef,
  onDrop,
}: UseColumnDragOld2Options): UseColumnDragOld2 {
  const [drag, setDrag] = useState<ColumnDragStateOld2 | null>(null);
  const pending = useRef<{ key: string; label: string; startX: number } | null>(null);
  const live = useRef<ColumnDragStateOld2 | null>(null);
  ...
  const place = useCallback(
    (key: string, label: string, x: number, y: number): ColumnDragStateOld2 | null => {
      const root = viewportRef.current;
      if (!root) return null;
      const ts = targets();
