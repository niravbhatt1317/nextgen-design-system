...
function pager(overrides: Partial<Parameters<typeof TablePagerOld2>[0]> = {}) {
  const onPage = vi.fn();
...
describe('TableLoadMoreOld2', () => {
  it('says how many of the total are showing', () => {
    render(<TableLoadMoreOld2 shown={25} total={1240} noun="people" onMore={vi.fn()} />);
    expect(screen.getByText('Showing 25 of 1,240 people')).toBeInTheDocument();
  });

  it('never claims to show more than there are', () => {
    render(<TableLoadMoreOld2 shown={99} total={40} noun="people" onMore={vi.fn()} />);
  ...
  it('loads more', async () => {
    ...
    const onMore = vi.fn();
    render(<TableLoadMoreOld2 shown={25} total={1240} noun="people" onMore={onMore} />);
    await user.click(screen.getByRole('button', { name: 'Load more' }));
    expect(onMore).toHaveBeenCalled();
  });

  it('offers nothing more once everything is shown', () => {
    render(<TableLoadMoreOld2 shown={40} total={40} noun="people" onMore={vi.fn()} />);
    expect(screen.queryByRole('button')).not.toBeInTheDocument();
  });

  it('waits while it loads', () => {
    render(<TableLoadMoreOld2 shown={25} total={1240} noun="people" loading onMore={vi.fn()} />);
