...
import { PersonCellOld2 } from './TableCells';
...
describe('DataTableOld2', { timeout: 20000 }, () => {
  it('draws the strip, 25 rows, the pager count with a comma-free total, and no chips', () => {
    ...
    expect(
      screen.getByRole('table', { name: 'Users' }).querySelectorAll('tbody tr.tbl-old2-row')
    ).toHaveLength(25);
  ...
  it('goes to a typed page and keeps the row numbers absolute', async () => {
    ...
    expect(
      screen.getByRole('table', { name: 'Users' }).querySelector('tbody tr .tbl-old2-num')
        ?.textContent
    ).toBe('51');
  });
  ...
  it('shows nothing-found with a Clear filters button that brings the rows back', async () => {
    ...
    expect(document.querySelector('.tbl-old2-foot')).toBeNull();
  ...
  it('keeps plain row numbers without selection, and lets the Columns panel hide them', async () => {
    ...
    expect(document.querySelector('.tbl-old2-cb')).toBeNull();
  ...
  it('shares spare width equally among the content columns, and only them', () => {
    ...
    Object.defineProperty(HTMLElement.prototype, 'clientWidth', {
      configurable: true,
      get() {
        return (this as HTMLElement).classList.contains('tbl-old2-viewport') ? 1600 : 0;
      },
    });
    try {
      render(<Users />);
