...
import type { TablePagingModeOld2 } from './TableOld2.types';

export interface UseTablePagingOld2Options {
  pageSize?: number | undefined;
  mode?: TablePagingModeOld2 | undefined;
}

export interface UseTablePagingOld2 {
  mode: TablePagingModeOld2;
  setMode: (mode: TablePagingModeOld2) => void;
  page: number;
  pageSize: number;
...
export function useTablePagingOld2({
  pageSize: initialSize = 25,
  mode: initialMode = 'pages',
}: UseTablePagingOld2Options = {}): UseTablePagingOld2 {
  const [mode, setMode] = useState<TablePagingModeOld2>(initialMode);
