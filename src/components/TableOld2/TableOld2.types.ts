...
export type TableSortDirectionOld2 = 'asc' | 'desc';

/** The sort that is on: which column and which way. `null` means unsorted. */
export interface TableSortStateOld2 {
  key: string;
  direction: TableSortDirectionOld2;
}
 ...
 */
export interface TableColumnDefOld2<Row> {
  ...
  align?: TableAlignOld2 | undefined;
...
export interface TableColumnsLayoutOld2 {
  order: string[];
  hidden: string[];
...
export type TableSelectionScopeOld2 = 'page' | 'all' | 'custom';

/** Pages, or a "Load more" footer that also fires as you scroll. Users keeps pages. */
export type TablePagingModeOld2 = 'pages' | 'loadMore';

/** The three blank states a list can be in. */
export type TableBlankKindOld2 = 'empty' | 'first' | 'error';

export interface TableOld2Props extends ComponentPropsWithoutRef<'div'> {
  ...
  /**
   * Let the table drive its own morph from the page scroll. **On by default.**
   *
   * The table takes the full height under the page’s dock line WHATEVER it
   * holds, so it expands on scroll even holding a single row, and its pager
   * stays on screen instead of floating up under a short list. (Pranjal’s
   * rule, and his default since 2026-09-10.)
   *
   * It needs a scrolling ancestor and a dock line to aim at. Inside a
   * `PageFrame` both are already there. With neither — a table in a drawer, a
   * modal or a card — nothing happens: it stays an ordinary card and keeps its
   * own max height, so the default is safe wherever the table lands.
   *
   * Give the table a `docked` value instead and the page keeps the job — this
   * stands down unless `expand` is also passed. `expand={false}` opts out.
   */
...
export interface TableViewportOld2Props extends ComponentPropsWithoutRef<'div'> {
...
export interface TableColGroupOld2Props {
...
export interface TableRowOld2Props extends ComponentPropsWithoutRef<'tr'> {
  /** The row is picked. */
...
export interface TableCellOld2Props extends ComponentPropsWithoutRef<'td'> {
...
export interface TableHeadOld2Props extends Omit<ComponentPropsWithoutRef<'th'>, 'onResize'> {
  ...
  sortable?: boolean | undefined;
  sort?: TableSortDirectionOld2 | null | undefined;
...
export interface TableSelectionCellOld2Props {
...
export interface TableNumberCellOld2Props {
  index: number;
...
export interface TableNumberHeadOld2Props {
  frozen?: number | undefined;
}

export interface TableSelectAllOld2Props {
...
export interface TableScopeMenuOld2Props {
  open: boolean;
  ...
  noun: string;
  onApply: (scope: TableSelectionScopeOld2, count: number) => void;
}

export interface TableBulkBarOld2Props extends ComponentPropsWithoutRef<'div'> {
...
export interface TableBulkActionOld2Props extends ComponentPropsWithoutRef<'button'> {
  icon?: ReactNode | undefined;
  /** A destructive action is red. */
  danger?: boolean | undefined;
}

export interface TablePagerOld2Props {
...
export interface TableLoadMoreOld2Props {
  shown: number;
  total: number;
  noun: string;
...
export interface TableBlankOld2Props {
  kind: TableBlankKindOld2;
...
export interface TableSkeletonOld2Props {
