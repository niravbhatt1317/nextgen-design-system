...
import type { TableColumnDefOld2, TableColumnsLayoutOld2 } from './TableOld2.types';

export interface UseTableColumnsOld2Options<Row> {
  /** The content columns, in their default order. */
  columns: TableColumnDefOld2<Row>[];
  /**
...
function load(storageKey: string | undefined): TableColumnsLayoutOld2 | null {
  if (!storageKey) return null;
  try {
    const raw = localStorage.getItem(storageKey);
    if (!raw) return null;
    const parsed = JSON.parse(raw) as Partial<TableColumnsLayoutOld2>;
    return {
...
function save(storageKey: string | undefined, layout: TableColumnsLayoutOld2): void {
 ...
 */
export function useTableColumnsOld2<Row>({
  columns,
  storageKey,
}: UseTableColumnsOld2Options<Row>): UseTableColumnsOld2<Row> {
  const keys = useMemo(() => columns.map((c) => c.key), [columns]);
  const byKey = useMemo(() => new Map(columns.map((c) => [c.key, c])), [columns]);
  const [layout, setLayout] = useState<TableColumnsLayoutOld2>(() => {
  ...
  const update = useCallback((fn: (l: TableColumnsLayoutOld2) => TableColumnsLayoutOld2) => {
    setLayout((l) => fn(l));
  }, []);
  ...
  const visible = useMemo(
    () =>
      layout.order
        ...
        .map((k) => byKey.get(k))
        .filter((c): c is TableColumnDefOld2<Row> => c !== undefined),
