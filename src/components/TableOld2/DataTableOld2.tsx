...
import {
  TABLE_COLUMN_MAX_OLD2,
  TABLE_COLUMN_MIN_OLD2,
  TABLE_COLUMN_WIDTH_OLD2,
  TABLE_GUTTER_OLD2,
  TableOld2,
  TableBodyOld2,
  TableCellOld2,
...
import { TableBulkBarOld2 } from './TableBulkBarOld2';
import { TableLoadMoreOld2, TablePagerOld2 } from './TablePagerOld2';
import { TableColumnsPanelOld2, TableInsertPanelOld2 } from './TablePanels';
import type { TableColumnsPanelColumnOld2 } from './TablePanels';
import type { UseTableColumnsOld2 } from './useTableColumnsOld2';
import { TableScopeMenuOld2 } from './TableScopeMenuOld2';
import { TableBlankOld2, TableSkeletonOld2 } from './TableStates';
import { useColumnDragOld2 } from './useColumnDragOld2';
import { useTableColumnsOld2 } from './useTableColumnsOld2';
import { useTablePagingOld2 } from './useTablePagingOld2';
import { useTableSelectionOld2 } from './useTableSelectionOld2';
import { useTableSortOld2 } from './useTableSortOld2';
import type { DataTableOld2Props } from './DataTableOld2.types';
import type { TableColumnDefOld2, TableSortDirectionOld2 } from './TableOld2.types';
...
function columnsPanel<Row>(opts: {
  ...
  layout: UseTableColumnsOld2<Row>;
  ...
  const own: TableColumnsPanelColumnOld2[] = layout.order.map((k) => ({
    key: k,
    label: labelOf(k),
    hidden: !isVisible(k),
  }));
  const rowNumber: TableColumnsPanelColumnOld2 = {
    key: ROWNUM_KEY,
    label: 'Row number',
    hidden: !numbers,
  };
...
function DataTableOld2<Row>({
  ...
  const sort = useTableSortOld2();
  const pagingState = useTablePagingOld2({ pageSize, mode: paging });
  const selection = useTableSelectionOld2();
  const layout = useTableColumnsOld2({ columns, storageKey });
  ...
  // ── derived rows ──
  const allColumns = useMemo<TableColumnDefOld2<Row>[]>(
    () => [
      {
        key: NAME_KEY,
  ...
  const nameWidth = Math.max(
    ...
    Math.min(
      ...
      layout.hasWidth(NAME_KEY)
        ...
        : (nameColumn.width ?? TABLE_COLUMN_WIDTH_OLD2)
    )
  );
  ...
  const widths = useMemo(() => {
    ...
    const covered = baseWidths.reduce((a, b) => a + b, 0) + TABLE_GUTTER_OLD2;
  ...
  const tailWidth = frozen
    ? Math.max(TABLE_GUTTER_OLD2, viewportWidth - columnsWidth)
    : TABLE_GUTTER_OLD2;
  ...
  const nameLeft = leading ? TABLE_GUTTER_OLD2 : 0;
  const actionLeft = nameLeft + nameWidth;

  // ── drag, insert ──
  const drag = useColumnDragOld2({
    viewportRef,
  ...
  const focusSibling = (tr: HTMLTableRowElement, direction: -1 | 1) => {
    ...
    if (next instanceof HTMLTableRowElement && next.classList.contains('tbl-old2-row'))
      next.focus();
  };
  ...
  const headMenu = (c: TableColumnDefOld2<Row>): ReactNode => (
    <>
      {c.sortable && (
        <>
          <DropdownMenuItem
  ...
  const sortDir = (key: string): TableSortDirectionOld2 | null =>
  ...
  return (
    <div className={cn('mdt-flex mdt-flex-col mdt-gap-4', className)}>
      ...
      <TableOld2
        ...
        divider={divider}
        docked={docked}
        style={tableMorphOfOld2(docked).driven ? { height: maxHeight } : undefined}
      >
        <TableViewportOld2
          ref={viewportRef}
          ...
          label={label}
        >
          <TableColGroupOld2 widths={widths} tail={tailWidth} />
          <TableHeaderOld2>
            <tr>
              {selectable && (
                <TableSelectAllOld2
              ...
              {!selectable && numbers && <TableNumberHeadOld2 frozen={0} />}
              <TableHeadOld2
                columnKey={NAME_KEY}
                label={nameColumn.label ?? 'Name'}
                width={nameWidth}
              ...
              {visible.map((c, i) => (
                <TableHeadOld2
                  ...
                  minWidth={c.minWidth ?? TABLE_COLUMN_MIN_OLD2}
                  align={c.align ?? 'left'}
                ...
                pageRows.map((row, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {!selectable && numbers && (
                        <TableNumberCellOld2 index={rowIndex(i)} inert={isInert} frozen={0} />
                      )}
                      <TableCellOld2 frozen={nameLeft} frozenEdge={!hasActions}>
                        {nameColumn.cell(row)}
                      </TableCellOld2>
                      {hasActions && (
                        <TableCellOld2 frozen={actionLeft} frozenEdge align="center">
                          <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                              <button
                                type="button"
