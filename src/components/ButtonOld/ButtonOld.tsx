...
import type {
  ...
  ButtonOldSize,
  SpinnerSize,
} from './ButtonOld.types';

/**
 * ButtonOld variants using Class Variance Authority (CVA)
 * Provides consistent styling with support for multiple variants, sizes, and states
 */
export const ButtonOldVariants = cva(
 ...
 */
const colorClasses: Record<ButtonOldColor, string> = {
...
type SizeKey = NonNullable<ButtonOldProps['size']>;
 ...
 */
const spinnerSizeMap: Record<ButtonOldSize, SpinnerSize> = {
  sm: 'xs',
  md: 'sm',
  lg: 'md',
};
 ...
 */
function LoadingSpinner({ size = 'md' }: Readonly<{ size?: ButtonOldSize }>) {
  return (
 ...
 */
function getSpinnerSizeFromButtonSize(size: ButtonOldProps['size']): ButtonOldSize {
...
function resolveAiSparkle(props: {
  variant: ButtonOldProps['variant'];
...
function computeButtonClasses(props: {
  color: ButtonOldColor | undefined;
  variant: ButtonOldProps['variant'];
  size: ButtonOldProps['size'];
  iconOnly: boolean;
  fullWidth: boolean | null | undefined;
  shape: ButtonOldProps['shape'];
  elevation: ButtonOldProps['elevation'];
  active: boolean;
  uppercase: boolean;
  error: boolean;
  success: boolean;
   ...
   */
  const iconOnlySizeClasses: Record<NonNullable<ButtonOldProps['size']>, string> = {
  ...
  const baseClasses = ButtonOldVariants({
...
function buildButtonContent(props: {
  badge?: ButtonOldProps['badge'];
  badgePosition: BadgePosition;
  loading: boolean;
  loadingPosition: ButtonOldProps['loadingPosition'];
  spinnerSize: ButtonOldSize;
  leftIcon?: React.ReactNode;
  rightIcon?: React.ReactNode;
  success: boolean;
  ...
  loadingText?: ButtonOldProps['loadingText'];
  successText?: ButtonOldProps['successText'];
...
function renderAsLink(props: {
  ...
  tooltipContent: ButtonOldProps['tooltipContent'];
...
/**
 * ButtonOld component with extensive features and variants.
 *
 * @example
 * ```tsx
 * // Basic usage
 * <ButtonOld variant="primary">Click me</ButtonOld>
 *
 * // With loading state
 * <ButtonOld loading loadingText="Saving...">Save</ButtonOld>
 *
 * // With icons
 * <ButtonOld leftIcon={<PlusIcon />}>Add Item</ButtonOld>
 *
 * // Icon only
 * <ButtonOld iconOnly shape="circle"><TrashIcon /></ButtonOld>
 *
 * // With badge
 * <ButtonOld badge={3}>Notifications</ButtonOld>
 *
 * // As link
 * <ButtonOld href="/dashboard" target="_blank">Dashboard</ButtonOld>
 *
 * // With tooltip
 * <ButtonOld tooltipContent="Click to save">Save</ButtonOld>
 * ```
 *
 * @deprecated Since 0.4.0. Use `Button`, now the merged console button.
 */
const ButtonOld = forwardRef<HTMLButtonElement | HTMLAnchorElement, ButtonOldProps>(
  (props, ref) => {
    const {
      className,
      style,
