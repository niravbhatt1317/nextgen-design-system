    ...
    docs: {
      description: {
        component: [
          ...
          '',
          'The dot is the strong tone colour, 6px, 8px at large. Every tone colour is a token,',
          '`--mdt-badge-<tone>-fill` / `-ink` / `-dot`, a full colour read with `var()`: light in',
          '`globals.css`, where the six console colours without a palette name are flagged, dark from',
          "the colour map's badge tier (`ai` is not in the map yet and rides the purple ramp, his",
          'ruling pending). `badge.css` holds no theme rule and no typed-in colour.',
        ].join('\n'),
      },
    },
  },
