...
import type { BadgeOldProps, BadgeOldTone } from './BadgeOld.types';
 ...
 */
export const badgeOldVariants = cva(
  [
 ...
 */
const DOT_TONE: Record<BadgeOldTone, string> = {
 ...
 * `<BadgeOld max={99}>1284</BadgeOld>` and `<BadgeOld max={99}>{1284}</BadgeOld>` look
 * identical to whoever writes them, so both are treated as counts. Anything
 * that is not a number is returned untouched.
 */
function capCount(children: BadgeOldProps['children'], max: number | undefined) {
...
/**
 * BadgeOld - a small label that says what something is.
 *
 * @example
 * ```tsx
 * <BadgeOld tone="success" dot>Active</BadgeOld>
 * <BadgeOld tone="info" shape="square">3 users</BadgeOld>
 * <BadgeOld tone="danger" emphasis="solid" max={99}>1284</BadgeOld>
 * <BadgeOld tone="danger" icon={<Icon name="x" />} aria-label="Failed" />
 * <BadgeOld tone="success" dot aria-label="Healthy" />
 * ```
 *
 * @deprecated Since 0.4.0. Use `Badge`, the badge ported from the merged console,
 * instead. This version stays only for side-by-side comparison until the removal
 * pull request.
 */
const BadgeOld = forwardRef<HTMLSpanElement, BadgeOldProps>(
  (
    {
      tone = 'neutral',
      emphasis = 'subtle',
    ...
    return (
      <span
        ref={ref}
        className={cn(
          badgeOldVariants({ tone, emphasis, shape, size, iconOnly }),
