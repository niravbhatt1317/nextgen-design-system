 ...
 */
function OptionPrefix({ option }: Readonly<{ option: SelectOld2Option }>) {
  return (
    <>
      {option.avatar && (
        <img
...
function OptionContent({
  option,
  asColumn = false,
}: Readonly<{ option: SelectOld2Option; asColumn?: boolean }>) {
  if (asColumn) {
    return (
...
/**
 * SelectOld2 trigger variants using Class Variance Authority (CVA)
 */
export const selectOld2TriggerVariants = cva(
  [
...
// ============================================================================
// Single SelectOld2 Mode Renderer - Extracted to reduce cognitive complexity
// ============================================================================

/**
 * Renders single select mode using Radix UI SelectOld2 primitive
 */
function renderSingleSelectMode(props: {
  currentValue: SelectOld2Value;
  ...
  maxHeight: number;
  sortedOptions: SelectOld2Option[];
    ...
    <div className={cn('mdt-flex mdt-flex-col mdt-gap-1.5', wrapperClassName)}>
      ...
      >
        <SelectPrimitive.Trigger
          ...
          className={cn(
            'mdt-group',
            selectOld2TriggerVariants({ variant, size, hasError }),
            className
          )}
...
// ============================================================================
// Multi-SelectOld2 Mode Helper Functions - Extracted to reduce cognitive complexity
// ============================================================================
...
function renderPill(
  ...
  _renderPillHoverCard?: (props: { option: SelectOld2Option }) => ReactNode
) {
  const pillContent = (
    <span
      key={option.value}
...
function renderSelectedValueDisplay(props: {
  ...
  handlePillRemove: (value: string) => void;
  _pillHoverCard: boolean;
  _renderPillHoverCard: ((props: { option: SelectOld2Option }) => ReactNode) | undefined;
  placeholder: string;
}): ReactNode {
  const {
    _showPills,
...
function renderPopoverDropdownContent(props: {
  ...
  maxHeight: number;
  _loading: boolean;
  displayOptions: SelectOld2Option[];
  renderEmpty?: () => ReactNode;
  emptyMessage: string;
  _grouped: boolean;
  groupedOptions: { label: string; options: SelectOld2Option[] }[] | null;
  multiValue: string[];
  _renderItem?: (props: {
    option: SelectOld2Option;
    selected: boolean;
...
function renderMultiSelectOptionsContent(props: {
  ...
  return (
    <>
      {(itemsToRender as SelectOld2Option[]).map((option, index) => {
        const isSelected = multiValue.includes(option.value.toString());
        const selectedCount = (itemsToRender as SelectOld2Option[]).filter((opt) =>
...
/**
 * Custom hook for managing SelectOld2 component state
 */
function useSelectState(props: {
  controlledValue: SelectOld2Value | undefined;
  defaultValue: string | string[] | undefined;
  mode: 'single' | 'multiple';
  onChange: ((value: SelectOld2Value) => void) | undefined;
}) {
  const { controlledValue, defaultValue, mode, onChange } = props;

  const initialValue = defaultValue ?? (mode === 'multiple' ? [] : null);
  const [internalValue, setInternalValue] = useState<SelectOld2Value>(initialValue);
...
function useMultiSelectHandlers(props: {
  ...
  handleValueChange: (value: SelectOld2Value) => void;
...
function useSelectSearch(props: {
  ...
  onLoadOptions: ((query: string) => Promise<SelectOld2Option[]>) | undefined;
...
function useOptionsProcessing(props: {
  ...
  const groupedOptions = useMemo(() => {
    ...
    const groups = new Map<string, SelectOld2Option[]>();
...
function useFilteredOptions(props: {
  options: SelectOld2Option[];
  searchQuery: string;
  filterFn: ((option: SelectOld2Option, query: string) => boolean) | undefined;
}) {
  ...
  return useMemo(() => {
    ...
    const defaultFilter = (option: SelectOld2Option, query: string) => {
...
function buildMultiSelectModeProps(params: {
  ...
  multiValue: string[];
  _renderItem:
    | ((props: { option: SelectOld2Option; selected: boolean; disabled: boolean }) => ReactNode)
    | undefined;
  handleMultiSelectToggle: (value: string) => void;
  _virtual: boolean;
  itemsToRender:
...
// ============================================================================
// Multi-SelectOld2 Mode Renderer - Extracted to reduce cognitive complexity
// ============================================================================
 ...
 * @deprecated Since 1.0.0. Use `Select` instead — the field rebuilt on the 2026-09-17 rulings.
 *
 ...
 * SelectOld2 component with single/multi-select support, search, pills, and advanced features.
 *
 * @example
 * ```tsx
 ...
 * ```
 */
const SelectOld2 = forwardRef<HTMLButtonElement, SelectOld2Props>(
  ...
  <T = any,>(
    ...
    // Single select: Radix UI SelectOld2 handles this natively
    if (mode === 'single') {
...
SelectOld2.displayName = 'SelectOld2';

// eslint-disable-next-line @typescript-eslint/no-explicit-any
const SelectOld2Component = SelectOld2 as <T = any>(
  props: SelectOld2Props<T> & { ref?: React.Ref<HTMLButtonElement> }
) => React.ReactElement;

export { SelectOld2Component as SelectOld2 };
