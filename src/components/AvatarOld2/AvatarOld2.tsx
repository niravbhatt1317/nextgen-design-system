...
import type { AvatarOld2Props, AvatarOld2Size, AvatarOld2Tone } from './AvatarOld2.types';

/** The six-colour palette, as a stable order so the hash below is repeatable. */
const TONES: AvatarOld2Tone[] = ['slate', 'blue', 'green', 'amber', 'rose', 'purple'];
 ...
 */
const INITIALS_FOR_SIZE: Record<AvatarOld2Size, number> = {
  xs: 1,
  sm: 1,
  md: MAX_INITIALS,
  lg: MAX_INITIALS,
 ...
 */
export const avatarOld2Variants = cva(
  [
...
const LARGE_ROUNDED: Partial<Record<AvatarOld2Size, string>> = {
  lg: 'mdt-rounded-lg',
  xl: 'mdt-rounded-lg',
};
 ...
 */
export const toneForNameOld2 = (name: string): AvatarOld2Tone => {
  let hash = 0;
 ...
 * @deprecated Since 1.0.0. Use `Avatar` instead — one initial at every size, and slate out of the rotation.
 *
 ...
 * AvatarOld2 - a person or thing, as a photo or their initials.
 *
 * @example
 * ```tsx
 * <AvatarOld2 name="Sarah Johnson" />
 * <AvatarOld2 name="Sarah Johnson" shape="rounded" size="lg" />
 * <AvatarOld2 name="Sarah Johnson" src="/sarah.jpg" />
 * ```
 */
const AvatarOld2 = forwardRef<HTMLSpanElement, AvatarOld2Props>(
  (
    ...
    const resolvedTone = tone ?? (name ? toneForNameOld2(name) : 'slate');
    ...
    const text = (initials ?? initialsForNameOld2(name)).slice(0, INITIALS_FOR_SIZE[size]);
    ...
    return (
      <span
        ref={ref}
        className={cn(
          avatarOld2Variants({ tone: resolvedTone, size, shape, ring }),
...
AvatarOld2.displayName = 'AvatarOld2';
