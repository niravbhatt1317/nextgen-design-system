...
import type { avatarOld2Variants } from './AvatarOld2';

export type AvatarOld2VariantsType = VariantProps<typeof avatarOld2Variants>;
 ...
 */
export type AvatarOld2Tone = 'slate' | 'blue' | 'green' | 'amber' | 'rose' | 'purple';

export type AvatarOld2Size = 'xs' | 'sm' | 'md' | 'lg' | 'xl';
 ...
 */
export type AvatarOld2Shape = 'circle' | 'rounded';

export interface AvatarOld2OwnProps {
  /**
  ...
  /** @default 'circle' */
  shape?: AvatarOld2Shape;

  /**
   * Draws a ring in the page background colour, so overlapping avatars stay
   * separated. Set automatically inside AvatarStackOld2.
   * @default false
   */
  ring?: boolean;
...
export type AvatarOld2Props = AvatarOld2OwnProps &
  Omit<ComponentPropsWithoutRef<'span'>, 'children' | 'className' | 'color'>;

export interface AvatarStackOld2Props {
