...
const Harness = ({ onMove }: { onMove: (key: string, to: number) => void }) => {
  const reorder = useColumnReorderOld({ columns, onMove });
  return (
    <TableOld>
      <TableHeaderOld>
        <TableRowOld>
...
describe('useColumnReorderOld', () => {
        ...
        <>
          ...
          <TableOld>
            <TableHeaderOld>
              <TableRowOld>
                {columns.map((column) => (
                  <TableHeadOld key={column.key} columnKey={column.key}>
                    {column.label}
                    <button type="button" {...reorder.gripProps(column.key)}>
                      grip
                    </button>
  ...
  describe('pinned columns', () => {
    const Pinned = ({ onMove }: { onMove: (key: string, to: number) => void }) => {
      const reorder = useColumnReorderOld({ columns, onMove, frozenCount: 2 });
      return (
        <TableOld>
          <TableHeaderOld>
            <TableRowOld>
