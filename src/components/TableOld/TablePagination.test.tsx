...
describe('TablePaginationOld', () => {
  describe('the count', () => {
    it('says which rows these are, out of how many', () => {
      render(<TablePaginationOld {...props} />);
    ...
    it('says nothing odd about an empty table', () => {
      render(<TablePaginationOld {...props} page={1} pageCount={1} from={0} to={0} total={0} />);
      expect(screen.getByText('0 of 0')).toBeInTheDocument();
    });

    it('stays when there is only one page', () => {
      render(<TablePaginationOld {...props} page={1} pageCount={1} from={0} to={8} total={8} />);
  ...
  describe('the pages', () => {
    ...
    it('goes to the page that was pressed', async () => {
      ...
      render(<TablePaginationOld {...props} onPageChange={onPageChange} />);
    ...
    it('disables the ends, for real', async () => {
      const user = userEvent.setup();
      const onPageChange = vi.fn();
      render(<TablePaginationOld {...props} page={1} onPageChange={onPageChange} />);
    ...
    it('disables the far end on the last page', () => {
      render(<TablePaginationOld {...props} page={5} />);
    ...
    it('gaps a long list rather than printing forty numbers', () => {
      render(<TablePaginationOld {...props} page={10} pageCount={40} />);
  ...
  describe('rows per page', () => {
    ...
    it('offers the sizes and reports the choice', async () => {
      ...
      render(<TablePaginationOld {...props} onPageSizeChange={onPageSizeChange} />);
    ...
    it('offers the sizes it was given', async () => {
      const user = userEvent.setup();
      render(
        <TablePaginationOld {...props} pageSizes={[5, 500]} onPageSizeChange={() => undefined} />
      );
      await user.click(screen.getByRole('combobox', { name: 'Rows per page' }));
