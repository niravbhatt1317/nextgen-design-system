...
describe('useTableFiltersOld', () => {
  it('starts with nothing filtered', () => {
    const { result } = renderHook(() => useTableFiltersOld<Key>());
  ...
  it('starts from what it was given', () => {
    const { result } = renderHook(() =>
      useTableFiltersOld<Key>({ initial: [{ attribute: 'status', values: ['Open'] }] })
    );
  ...
  it('survives a re-render', () => {
    const { result, rerender } = renderHook(() => useTableFiltersOld<Key>());
    act(() => {
      result.current.toggleValue('status', 'Open');
    });
    rerender();
