...
describe('useTableSelectionOld', () => {
  describe('starting state', () => {
    it('starts with nothing selected', () => {
      const { result } = renderHook(() => useTableSelectionOld({ rowIds }));
    ...
    it('starts from what it was given', () => {
      const { result } = renderHook(() => useTableSelectionOld({ rowIds, initial: ['b'] }));
  ...
  describe('one row at a time', () => {
    ...
    it('reports some, then all', () => {
      const { result } = renderHook(() => useTableSelectionOld({ rowIds: ['a', 'b'] }));
      act(() => {
  ...
  describe('shift-click', () => {
    ...
    it('falls back to a plain toggle when the anchor has gone', () => {
      const { result, rerender } = renderHook(({ ids }) => useTableSelectionOld({ rowIds: ids }), {
