 ...
 */
const meta: Meta = {
  title: 'Deprecated/Table Old Column Controls',
  tags: ['autodocs'],
...
const definitions: TableColumnDefOld<Key>[] = [
...
export const ColumnControls: Story = {
  render: function ColumnControlsDemo() {
    ...
    const { widths, setWidth } = useColumnWidthsOld({
      id: 88,
      subject: 236,
      status: 120,
      priority: 81,
        ...
        <TableOld containerClassName="mdt-rounded-md mdt-border">
          ...
          <TableBodyOld>
            {tickets.map((ticket) => (
              <TableRowOld key={ticket.id}>
                {cols.visible.map((column) => (
                  <TableCellOld key={column.key} className="mdt-whitespace-nowrap">
                    {cellFor(ticket, column.key)}
                  </TableCellOld>
                ))}
              </TableRowOld>
