...
import type { TableColumnDefOld } from './useTableColumns';

const defs: TableColumnDefOld<'id' | 'subject' | 'priority' | 'assignee'>[] = [
...
describe('useTableColumnsOld', () => {
  describe('starting state', () => {
    it('starts in the order it was given, all visible', () => {
      const { result } = renderHook(() => useTableColumnsOld(defs));
  ...
  describe('freezing', () => {
    ...
    it('respects a different limit', () => {
      const { result } = renderHook(() => useTableColumnsOld(defs, { maxFrozen: 1 }));
  ...
  describe('reset and restore', () => {
    ...
    it('survives a re-render with the same definitions', () => {
      const { result, rerender } = renderHook(({ d }) => useTableColumnsOld(d), {
        initialProps: { d: defs },
      });
      act(() => {
        result.current.hide('priority');
