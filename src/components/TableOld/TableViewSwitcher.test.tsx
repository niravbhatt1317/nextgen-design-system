...
describe('TableViewSwitcherOld', () => {
  describe('the trigger', () => {
    it('shows the label with no view active', () => {
      render(<TableViewSwitcherOld views={views} onApply={noop} onSaveAs={noop} />);
      expect(screen.getByRole('button', { name: /Views/ })).toBeInTheDocument();
    });

    it('shows the active view instead', () => {
      render(<TableViewSwitcherOld views={views} activeId="b" onApply={noop} onSaveAs={noop} />);
      expect(screen.getByRole('button', { name: /Mine/ })).toBeInTheDocument();
    });

    it('marks unsaved changes', () => {
      render(
        <TableViewSwitcherOld views={views} activeId="b" dirty onApply={noop} onSaveAs={noop} />
      );
  ...
  describe('the list', () => {
    it('applies the view that was picked', async () => {
      ...
      const onApply = vi.fn();
      render(<TableViewSwitcherOld views={views} onApply={onApply} onSaveAs={noop} />);
      await open(user);
    ...
    it('says so when there is nothing saved', async () => {
      const user = userEvent.setup();
      render(<TableViewSwitcherOld views={[]} onApply={noop} onSaveAs={noop} />);
      await open(user);
    ...
    it('deletes without applying the row it was on', async () => {
      ...
      const onApply = vi.fn();
      render(
        <TableViewSwitcherOld views={views} onApply={onApply} onSaveAs={noop} onRemove={onRemove} />
      );
      await open(user);
  ...
  describe('naming', () => {
    it('saves the table as a new view', async () => {
      ...
      render(<TableViewSwitcherOld views={views} onApply={noop} onSaveAs={onSaveAs} />);
      await open(user);
    ...
    it('renames from the row, starting with the name it has', async () => {
      ...
      render(
        <TableViewSwitcherOld views={views} onApply={noop} onSaveAs={noop} onRename={onRename} />
      );
      await open(user);
  ...
  describe('unsaved changes', () => {
    ...
    it('says nothing about changes with no view to have changed', async () => {
      const user = userEvent.setup();
      render(
        <TableViewSwitcherOld views={views} dirty onApply={noop} onSaveAs={noop} onSave={noop} />
      );
      await open(user);
