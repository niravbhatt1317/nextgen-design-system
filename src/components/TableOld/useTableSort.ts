...
export type SortDirectionOld = 'ascend' | 'descend';

/** One column's contribution to the order. */
export interface SortRuleOld<K extends string = string> {
  column: K;
  direction: SortDirectionOld;
}

export interface UseTableSortOldOptions<K extends string = string> {
  /** Where the table starts out. Captured once. */
  initial?: SortRuleOld<K>[];
...
export interface UseTableSortOld<K extends string = string> {
  /**
  ...
  sortBy: (column: K, direction: SortDirectionOld) => void;
   ...
   */
  restore: (rules: SortRuleOld<K>[]) => void;

  /** Which way a column is sorted, or `null` if it is not. */
  directionOf: (column: K) => SortDirectionOld | null;
...
const flip = (direction: SortDirectionOld): SortDirectionOld =>
  direction === 'ascend' ? 'descend' : 'ascend';

/**
 * Holds which columns sort the table, and in what order.
 *
 * Like the rest of TableOld it **holds state and never touches your rows**. It
 ...
 * @example
 * ```tsx
 * const sort = useTableSortOld<ColumnKey>();
 *
 * <TableHeadOld
 *   sortable
 ...
 */
export function useTableSortOld<K extends string>({
  initial = [],
  multiple = true,
}: UseTableSortOldOptions<K> = {}): UseTableSortOld<K> {
  // Captured once, like the other TableOld hooks: re-reading the argument every
  // render would undo a sort the moment the caller re-rendered for any other
  // reason.
  const [start] = useState<SortRuleOld<K>[]>(initial);
  const [rules, setRules] = useState<SortRuleOld<K>[]>(start);
  ...
  const sortBy = useCallback(
    (column: K, direction: SortDirectionOld) => {
      setRules((prev) => {
  ...
  const restore = useCallback((next: SortRuleOld<K>[]) => {
