...
const stored: TableViewOld<State>[] = [
...
const setup = (current: State = { sort: 'created' }, options = {}) =>
  renderHook(
    ({ live }) => useSavedViewsOld<State>({ current: live, initial: stored, ...options }),
    {
      initialProps: { live: current },
    }
  );

describe('useSavedViewsOld', () => {
  describe('starting state', () => {
    ...
    it('starts empty', () => {
      const { result } = renderHook(() =>
        useSavedViewsOld<State>({ current: { sort: 'created' } })
      );
    ...
    it('ignores a later change to the initial list', () => {
      const { result, rerender } = renderHook(
        ({ initial }) => useSavedViewsOld<State>({ current: { sort: 'created' }, initial }),
        { initialProps: { initial: stored } }
      );
      rerender({ initial: [] });
      // Captured once, like every other TableOld hook: re-reading it would wipe a
