...
import { PAGE_GAPOld, pageListOld } from './pageList';
import type { TablePaginationOldProps } from './TableOld.types';

const DEFAULT_PAGE_SIZES = [10, 25, 50, 100];

/**
 * TablePaginationOld - the pager under a table.
 *
 ...
 *
 * **Rows per page.** `useTablePaginationOld.setPageSize` keeps the first visible
 ...
 *
 * Like everything else under `TableOld` it holds no state: it takes the numbers
 * and reports the presses. `useTablePaginationOld` is what usually feeds it, and a
 * table backed by an API can feed it from a response instead.
 *
 * @example
 * ```tsx
 * const pagination = useTablePaginationOld({ total: rows.length, pageSize: 25 });
 * <TablePaginationOld {...pagination} total={rows.length} onPageChange={pagination.goTo} />
 * ```
 *
 ...
 */
const TablePaginationOld = forwardRef<HTMLDivElement, TablePaginationOldProps>(
  (
    {
      page,
      pageCount,
      ...
      {pageCount > 1 && (
        <Pagination className="mdt-w-auto mdt-justify-end">
          <PaginationContent>
            ...
            {pageListOld(page, pageCount).map((slot, index) =>
              slot === PAGE_GAPOld ? (
...
TablePaginationOld.displayName = 'TablePaginationOld';

export { TablePaginationOld };
