...
describe('useColumnWidthsOld', () => {
  it('starts at the widths it was given', () => {
    const { result } = renderHook(() => useColumnWidthsOld({ name: 200, email: 260 }));
  ...
  it('clamps to the minimum - a column that reaches zero cannot be grabbed again', () => {
    const { result } = renderHook(() => useColumnWidthsOld({ name: 200 }, { minWidth: 80 }));
    act(() => {
  ...
  it('clamps to the maximum', () => {
    const { result } = renderHook(() => useColumnWidthsOld({ name: 200 }, { maxWidth: 400 }));
    act(() => {
  ...
  it('rounds to whole pixels', () => {
    const { result } = renderHook(() => useColumnWidthsOld({ name: 200 }));
    act(() => {
  ...
  it('keeps a resize when the caller re-renders for another reason', () => {
    ...
    const { result, rerender } = renderHook(({ init }) => useColumnWidthsOld(init), {
