...
describe('useInfiniteScrollOld', () => {
  it('watches the sentinel once it is there', () => {
    const { result } = renderHook(() =>
      useInfiniteScrollOld({ hasMore: true, onLoadMore: vi.fn() })
    );
  ...
  it('asks for more when the end comes into view', () => {
    ...
    const { result } = renderHook(() => useInfiniteScrollOld({ hasMore: true, onLoadMore }));
    act(() => {
      result.current.sentinelRef(node());
    });
    scrollIntoView();
  ...
  describe('when it stays quiet', () => {
    ...
    it.each(cases)('does not watch when $name', ({ options }) => {
      const onLoadMore = vi.fn();
      const { result } = renderHook(() => useInfiniteScrollOld({ ...options, onLoadMore }));
      act(() => {
        result.current.sentinelRef(node());
      });
  ...
  it('starts watching again once the load lands', () => {
    ...
    const { result, rerender } = renderHook(
      ({ loading }) => useInfiniteScrollOld({ hasMore: true, loading, onLoadMore }),
  ...
  it('stops watching when the rows run out', () => {
    const { result, rerender } = renderHook(
      ({ hasMore }) => useInfiniteScrollOld({ hasMore, onLoadMore: vi.fn() }),
  ...
  it('keeps one observer when the callback changes every render', () => {
    const { result, rerender } = renderHook(() =>
      useInfiniteScrollOld({ hasMore: true, onLoadMore: () => undefined })
    );
    act(() => {
      result.current.sentinelRef(node());
    });
  ...
  it('calls the newest callback, not the one it was created with', () => {
    ...
    const second = vi.fn();
    const { result, rerender } = renderHook(
      ({ onLoadMore }) => useInfiniteScrollOld({ hasMore: true, onLoadMore }),
