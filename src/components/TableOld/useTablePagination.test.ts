...
describe('useTablePaginationOld', () => {
  it('starts on the first page', () => {
    const { result } = renderHook(() => useTablePaginationOld({ total: 100, pageSize: 10 }));
  ...
  it('reports the range of rows on this page', () => {
    const { result } = renderHook(() => useTablePaginationOld({ total: 95, pageSize: 10 }));
    act(() => {
  ...
  it('moves back and forward', () => {
    const { result } = renderHook(() => useTablePaginationOld({ total: 30, pageSize: 10 }));
    act(() => {
      result.current.next();
    });
  ...
  it('will not walk off either end', () => {
    const { result } = renderHook(() => useTablePaginationOld({ total: 20, pageSize: 10 }));
    act(() => {
      result.current.previous();
    });
    expect(result.current.page).toBe(1);
  ...
  it('keeps one page even with no rows', () => {
    const { result } = renderHook(() => useTablePaginationOld({ total: 0 }));
  ...
  it('takes this page out of an array', () => {
    ...
    const { result } = renderHook(() => useTablePaginationOld({ total: 25, pageSize: 10 }));
    act(() => {
  ...
  it('falls back to the last page when the table shrinks underneath it', () => {
    const { result, rerender } = renderHook(
      ({ total }) => useTablePaginationOld({ total, pageSize: 10 }),
      {
  ...
  it('keeps the first visible row on screen when the size changes', () => {
    const { result } = renderHook(() => useTablePaginationOld({ total: 1000, pageSize: 10 }));
    act(() => {
  ...
  it('refuses a page size of zero, which would divide by nothing', () => {
    const { result } = renderHook(() => useTablePaginationOld({ total: 50 }));
    act(() => {
  ...
  it('starts where it is told', () => {
    const { result } = renderHook(() =>
      useTablePaginationOld({ total: 100, pageSize: 10, initialPage: 4 })
    );
    expect(result.current.page).toBe(4);
