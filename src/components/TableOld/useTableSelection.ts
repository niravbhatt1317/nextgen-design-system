...
export type SelectionStateOld = boolean | 'indeterminate';

export interface UseTableSelectionOldOptions<Id extends string | number = string> {
  /**
...
export interface UseTableSelectionOld<Id extends string | number = string> {
  /**
   ...
   */
  headerState: SelectionStateOld;
}

/**
 * Holds which rows are selected.
 *
 * Like the rest of TableOld it holds state and never touches your rows: it reports
 * which ids are selected, and what happens to them - a bulk delete, an export,
 * an assignment - is the product's business.
 *
 * @example
 * ```tsx
 * const selection = useTableSelectionOld({ rowIds: rows.map((row) => row.id) });
 *
 * <Checkbox
 ...
 */
export function useTableSelectionOld<Id extends string | number>({
  rowIds,
  initial = [],
}: UseTableSelectionOldOptions<Id>): UseTableSelectionOld<Id> {
  ...
  const headerState = useMemo<SelectionStateOld>(() => {
