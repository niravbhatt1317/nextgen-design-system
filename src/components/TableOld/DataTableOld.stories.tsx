...
import { TableBulkActionOld, TableBulkSeparatorOld } from './TableBulkBar';

/**
 * The whole table in one component.
 *
 * Layer 5f. Everything under `TableOld` is deliberately state-free - `TableHeadOld`
 * reports that a sort was asked for, `useTableSortOld` remembers it, and neither
 ...
 *
 * `DataTableOld` does the work. **Every piece of it can be handed back**:
 ...
 */
const meta: Meta = {
  title: 'Deprecated/DataTable Old',
  tags: ['autodocs'],
 ...
 *
 * The pager is `TablePaginationOld`, which is the shared `Pagination` component
  ...
  render: () => (
    <DataTableOld
      ...
      bulkActions={(selected) => (
        <>
          <TableBulkActionOld icon={<Icon name="user-plus" size="sm" aria-hidden />}>
            Assign
          </TableBulkActionOld>
          <TableBulkActionOld icon={<Icon name="check" size="sm" aria-hidden />}>
            Resolve
          </TableBulkActionOld>
          <TableBulkSeparatorOld />
          <TableBulkActionOld icon={<Icon name="trash-2" size="sm" aria-hidden />}>
 ...
 *
 * This is what a table backed by a paged API looks like: `DataTableOld` renders and
