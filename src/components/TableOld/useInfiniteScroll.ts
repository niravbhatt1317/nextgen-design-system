...
export interface UseInfiniteScrollOldOptions {
  /**
...
export interface UseInfiniteScrollOld {
  /**
 ...
 * to be filed, so `DataTableOld` shows one or the other.
 *
 ...
 * @example
 * ```tsx
 * const { sentinelRef } = useInfiniteScrollOld({ hasMore, loading, onLoadMore });
 ...
 */
export function useInfiniteScrollOld({
  hasMore,
  loading = false,
  onLoadMore,
  rootMargin = DEFAULT_ROOT_MARGIN,
  disabled = false,
}: UseInfiniteScrollOldOptions): UseInfiniteScrollOld {
