...
import { TableOld, TableHeadOld, TableHeaderOld, TableRowOld } from './TableOld';
...
const renderHead = (props: Record<string, unknown>) =>
  render(
    <TableOld>
      <TableHeaderOld>
        <TableRowOld>
          <TableHeadOld {...props}>Subject</TableHeadOld>
        </TableRowOld>
      </TableHeaderOld>
    </TableOld>
  );
...
describe('TableColumnBoundaryOld', () => {
  ...
  describe('edge cases', () => {
    ...
    it('accepts extra classes when used directly', () => {
      // `TableHeadOld` sends its own `className` to the `<th>`, so this is only
      // reachable through the component itself - which is exported, so it is
      // a real path rather than a test-only one.
      render(<TableColumnBoundaryOld resizable width={200} className="custom-line" />);
    ...
    const renderRich = () =>
      render(
        <TableOld>
          <TableHeaderOld>
            <TableRowOld>
              <TableHeadOld resizable width={200} insertColumns={columns} onInsert={vi.fn()}>
                <em>Subject</em>
              </TableHeadOld>
            </TableRowOld>
          </TableHeaderOld>
