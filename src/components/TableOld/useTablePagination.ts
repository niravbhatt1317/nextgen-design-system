...
export interface UseTablePaginationOldOptions {
  /**
...
export interface UseTablePaginationOld {
 ...
 * @example
 * ```tsx
 * const pagination = useTablePaginationOld({ total: rows.length, pageSize: 10 });
 ...
 */
export function useTablePaginationOld({
  total,
  pageSize: initialSize = DEFAULT_PAGE_SIZE,
  initialPage = 1,
}: UseTablePaginationOldOptions): UseTablePaginationOld {
