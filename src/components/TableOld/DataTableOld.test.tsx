...
describe('DataTableOld', () => {
  describe('rendering', () => {
    it('renders a row per record and a column per definition', () => {
      render(<DataTableOld columns={columns} rows={rows} getRowId={(row) => row.id} />);
  ...
  describe('searching', () => {
    ...
    it('can be turned off entirely', () => {
      render(
        <DataTableOld columns={columns} rows={rows} getRowId={(row) => row.id} searchable={false} />
      );
  ...
  describe('awkward values', () => {
    ...
    it('prints a date rather than [object Object]', () => {
      render(<DataTableOld columns={oddColumns} rows={odd} getRowId={(row) => row.id} />);
  ...
  describe('paging', () => {
    ...
    it('shows one page at a time', () => {
      render(
        <DataTableOld columns={columns} rows={many} getRowId={(row) => row.id} pageSize={5} />
      );
    ...
    it('hides the controls when everything fits on one page', () => {
      render(
        <DataTableOld columns={columns} rows={rows} getRowId={(row) => row.id} pageSize={25} />
      );
  ...
  describe('selection', () => {
    ...
    it('selects a row and shows the bar', async () => {
      render(<DataTableOld columns={columns} rows={rows} getRowId={(row) => row.id} selectable />);
  ...
  describe('extras', () => {
    ...
    it('accepts extra classes', () => {
      const { container } = render(
        <DataTableOld columns={columns} rows={rows} getRowId={(row) => row.id} className="custom" />
      );
  ...
  describe('loading', () => {
    it('draws skeleton rows while the first rows are on their way', () => {
      render(<DataTableOld columns={columns} rows={[]} getRowId={(row: Row) => row.id} loading />);
    ...
    it('keeps the rows it has and marks them busy instead', () => {
      render(<DataTableOld columns={columns} rows={rows} getRowId={(row) => row.id} loading />);
    ...
    it('does not claim the table is empty while it is still loading', () => {
      const { rerender } = render(
        <DataTableOld columns={columns} rows={[]} getRowId={(row: Row) => row.id} loading />
      );
      expect(screen.queryByText('Nothing to show.')).not.toBeInTheDocument();
      rerender(<DataTableOld columns={columns} rows={[]} getRowId={(row: Row) => row.id} />);
  ...
  describe('infinite scroll', () => {
    ...
    it('shows nothing to load without somewhere to ask', () => {
      render(
        <DataTableOld columns={columns} rows={rows} getRowId={(row) => row.id} infinite hasMore />
      );
      expect(screen.queryByRole('button', { name: /Load more/ })).not.toBeInTheDocument();
    });
  });
  ...
  describe('rows per page', () => {
    ...
    it('is not offered unless asked for', () => {
      render(
        <DataTableOld columns={columns} rows={many} getRowId={(row) => row.id} pageSize={10} />
      );
