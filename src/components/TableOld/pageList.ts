...
export const PAGE_GAPOld = 'gap';

export type PageSlotOld = number | typeof PAGE_GAPOld;
 ...
 * @example
 * ```ts
 * pageListOld(1, 5)    // [1, 2, 3, 4, 5]
 * pageListOld(1, 20)   // [1, 2, 3, 4, 5, 'gap', 20]
 * pageListOld(10, 20)  // [1, 'gap', 9, 10, 11, 'gap', 20]
 * pageListOld(20, 20)  // [1, 'gap', 16, 17, 18, 19, 20]
 * ```
 */
export function pageListOld(page: number, pageCount: number): PageSlotOld[] {
  ...
  if (current <= EDGE) return [...range(1, MAX_SLOTS - 2), PAGE_GAPOld, pageCount];

  // Near the end: the mirror of it.
  if (current >= pageCount - EDGE + 1) {
    return [1, PAGE_GAPOld, ...range(pageCount - (MAX_SLOTS - 3), pageCount)];
  }

  return [1, PAGE_GAPOld, ...range(current - AROUND, current + AROUND), PAGE_GAPOld, pageCount];
}
