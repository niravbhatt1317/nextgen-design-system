 ...
 */
export interface TableViewOld<State> {
...
export interface UseSavedViewsOldOptions<State> {
  /**
   * The views to start with. Captured once, like every other TableOld hook.
   *
   ...
   * whole list anyway.
   */
  onChange?: (views: TableViewOld<State>[]) => void;
}

export interface UseSavedViewsOld<State> {
  /** Every saved view, in the order they were created. */
  views: TableViewOld<State>[];

  /** The view being looked at, or `null` for none. */
  active: TableViewOld<State> | null;
   ...
   */
  saveAs: (name: string) => TableViewOld<State>;
 ...
 * worth something.
 *
 * Like the rest of TableOld, **it holds state and never touches your rows**, and
 ...
 * const views = useSavedViewsOld({ current, initial: stored });
 *
 * // Applying one:
 ...
 */
export function useSavedViewsOld<State>({
  initial = [],
  initialActiveId = null,
  current,
  isSame = sameByJson,
  onChange,
}: UseSavedViewsOldOptions<State>): UseSavedViewsOld<State> {
  const [start] = useState<TableViewOld<State>[]>(initial);
  const [views, setViews] = useState<TableViewOld<State>[]>(start);
  ...
  const saveAs = useCallback(
    (name: string) => {
      const view: TableViewOld<State> = { id: mintId(), name, state: current };
      write([...views, view]);
      setActiveId(view.id);
      return view;
    },
