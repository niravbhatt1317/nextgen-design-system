 ...
 */
export interface TableColumnDefOld<K extends string = string> {
 ...
 */
export interface TableColumnsStateOld<K extends string = string> {
...
export interface UseTableColumnsOldOptions {
  /**
...
export interface TableColumnViewOld<K extends string = string> extends TableColumnDefOld<K> {
 ...
 *
 * Like the rest of TableOld, it holds state and never touches your rows. Hiding a
 * column does not filter anything; it tells you not to render that column.
 *
 * @example
 * ```tsx
 * const cols = useTableColumnsOld([
 ...
 *     <TableHeadOld key={column.key} frozen={column.frozen}>
 *       {column.label}
 *     </TableHeadOld>
 *   ))}
 * </TableRowOld>
 ...
 */
export function useTableColumnsOld<K extends string>(
  definitions: TableColumnDefOld<K>[],
  options: UseTableColumnsOldOptions = {}
): UseTableColumnsOld<K> {
  ...
  // `useColumnWidthsOld` avoids.
  const [defs] = useState(definitions);
  const [initial] = useState<TableColumnsStateOld<K>>(() => ({
    order: definitions.map((column) => column.key),
    hidden: [],
    frozenCount: 0,
  }));
  const [state, setState] = useState<TableColumnsStateOld<K>>(initial);

  const byKey = useMemo(() => new Map(defs.map((column) => [column.key, column])), [defs]);

  const columns = useMemo<TableColumnViewOld<K>[]>(() => {
    ...
    return state.order.flatMap((key) => {
      ...
      const view: TableColumnViewOld<K> = {
        ...def,
        visible: !isHidden,
  ...
  const orderIndexAt = useCallback((current: TableColumnsStateOld<K>, visibleIndex: number) => {
