...
describe('TableColumnMenuOld', () => {
  describe('which items appear', () => {
    it('shows only the items it was given a handler for', async () => {
      render(<TableColumnMenuOld label="Subject" onSort={vi.fn()} onHide={vi.fn()} />);
      await openMenu();
    ...
    it('falls back to plain text when there is nothing to offer', () => {
      render(<TableColumnMenuOld label="Subject" />);
      expect(screen.queryByRole('button')).not.toBeInTheDocument();
      expect(screen.getByText('Subject')).toBeInTheDocument();
    });

    it('renders a trigger when only a header was given', async () => {
      render(<TableColumnMenuOld label="Subject" header={<span>Sorting order</span>} />);
      await openMenu();
    ...
    it('renders custom trigger content', () => {
      render(
        <TableColumnMenuOld label="Subject" onSort={vi.fn()}>
  ...
  describe('interactions', () => {
    it('calls the handler for the chosen item', async () => {
      const onHide = vi.fn();
      render(<TableColumnMenuOld label="Subject" onHide={onHide} />);
      await openMenu();
    ...
    it('calls the freeze handler', async () => {
      ...
      render(<TableColumnMenuOld label="Subject" canFreeze onToggleFreeze={onToggleFreeze} />);
      await openMenu();
    ...
    it('opens on the keyboard', async () => {
      render(<TableColumnMenuOld label="Subject" onSort={vi.fn()} />);
