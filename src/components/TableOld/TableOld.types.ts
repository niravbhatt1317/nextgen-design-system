...
import type { TableColumnsStateOld } from './useTableColumns';
import type { TableFilterOld } from './useTableFilters';
import type { TableViewOld } from './useSavedViews';
import type { SortRuleOld } from './useTableSort';
 ...
 */
export type TableDensityOld = 'short' | 'compact' | 'default' | 'relaxed';
 ...
 * than any restyle.
 */
export type TableAlignOld = 'left' | 'center' | 'right';
 ...
 */
export type TableSortOrderOld = 'ascend' | 'descend' | null;

/**
 * What `TableOld` hands down to the cells inside it.
 *
...
/**
 * Props for the TableOld component
 */
export interface TableOldProps extends ComponentPropsWithoutRef<'table'> {
  /**
...
/**
 * Props for the TableHeaderOld component
 */
// eslint-disable-next-line @typescript-eslint/no-empty-object-type
export interface TableHeaderOldProps extends ComponentPropsWithoutRef<'thead'> {}

/**
 * Props for the TableBodyOld component
 */
// eslint-disable-next-line @typescript-eslint/no-empty-object-type
export interface TableBodyOldProps extends ComponentPropsWithoutRef<'tbody'> {}

/**
 * Props for the TableFooterOld component
 */
// eslint-disable-next-line @typescript-eslint/no-empty-object-type
export interface TableFooterOldProps extends ComponentPropsWithoutRef<'tfoot'> {}
 ...
 */
export type TableIndentOld = 0 | 1 | 2 | 3;

/**
 * Props for the TableRowOld component
 */
export interface TableRowOldProps extends ComponentPropsWithoutRef<'tr'> {
  /**
   ...
   *
   * `TableFooterOld` already treats its rows this way, so this is for a summary
...
/**
 * Props for the TableGroupRowOld component
 */
export interface TableGroupRowOldProps extends ComponentPropsWithoutRef<'tr'> {
  /**
...
/**
 * Props for the TableExpandTriggerOld component
 */
export interface TableExpandTriggerOldProps extends ComponentPropsWithoutRef<'button'> {
...
/**
 * Props for the TableHeadOld component
 */
export interface TableHeadOldProps extends ComponentPropsWithoutRef<'th'> {
  /**
   ...
   * behaviour are provided here, the width itself stays yours. `useColumnWidthsOld`
...
/**
 * Props for the TableCellOld component
 */
export interface TableCellOldProps extends ComponentPropsWithoutRef<'td'> {
  /**
...
/**
 * Props for the TableCaptionOld component
 */
// eslint-disable-next-line @typescript-eslint/no-empty-object-type
export interface TableCaptionOldProps extends ComponentPropsWithoutRef<'caption'> {}

/**
 * Props for TableColumnMenuOld - the controls belonging to one column.
 */
export interface TableColumnMenuOldProps {
...
/**
 * Props for TableColumnBoundaryOld - the line between two columns.
 */
export interface TableColumnBoundaryOldProps {
  ...
  /**
   * The columns available to add here. Usually `useTableColumnsOld().hidden`.
   *
...
/**
 * Props for TableToolbarOld - the controls that act on the whole table.
 */
export interface TableToolbarOldProps extends ComponentPropsWithoutRef<'div'> {
  /**
   * What a screen reader calls the group.
   *
   * A page can hold more than one table, and "TableOld controls" three times over
   * identifies none of them.
   *
   * @default 'TableOld controls'
   */
  label?: string;
}

/** Props for TableToolbarActionsOld - the trailing group of a toolbar. */
export type TableToolbarActionsOldProps = ComponentPropsWithoutRef<'div'>;

/**
 * Props for TableSortMenuOld.
 *
 * The rules come in and the changes go out: like everything else in TableOld this
 * renders a sort, it does not own one. `useTableSortOld` is the other half.
 */
export interface TableSortMenuOldProps {
...
/**
 * Props for TableViewMenuOld - grouping and which columns exist.
 */
export interface TableViewMenuOldProps {
...
/**
 * Props for TableBulkBarOld - what you can do with the rows you have selected.
 */
export interface TableBulkBarOldProps extends ComponentPropsWithoutRef<'div'> {
  ...
  /** The actions, usually `TableBulkActionOld`. */
  children?: ReactNode;
}

/** Props for TableBulkActionOld - one action on the bulk bar. */
export interface TableBulkActionOldProps extends ComponentPropsWithoutRef<'button'> {
  /** The glyph, before the label. */
  icon?: ReactNode;
}

/**
 * Props for TableFilterMenuOld - choose an attribute, then choose its values.
 */
export interface TableFilterMenuOldProps {
...
/**
 * Props for TableFilterChipsOld - what is currently narrowing the table.
 */
export interface TableFilterChipsOldProps extends ComponentPropsWithoutRef<'div'> {
  /** The active filters. */
...
/**
 * Props for DataTableOld - the whole table, assembled.
 */
export interface DataTableOldProps<Row> extends Omit<ComponentPropsWithoutRef<'div'>, 'children'> {
   ...
   * avatar. `DataTableOld` knows the shape of a table, not the meaning of your
   * data.
   */
   ...
   * given; `DataTableOld` becomes a renderer and reports what the user asked for
  ...
  initialViews?: TableViewOld<DataTableViewStateOld>[];
   ...
   *
   * Where persistence happens. `DataTableOld` deliberately writes nothing itself;
   * views shared between colleagues and views in a URL are both impossible for
   * a component that assumed `localStorage`.
   */
  onViewsChange?: (views: TableViewOld<DataTableViewStateOld>[]) => void;
}

/** One row of the switcher's list. The state a view holds is not its business. */
export interface TableViewSummaryOld {
  id: string;
  name: string;
}

export interface TableViewNamePanelOldProps {
...
export interface TableViewSwitcherOldProps {
  /** The saved views, in the order to list them. */
  views: TableViewSummaryOld[];
   ...
   * `useSavedViewsOld` does exactly that.
   *
   * @default false
   */
  dirty?: boolean;
...
/**
 * What a `DataTableOld` saves when you save a view.
 *
 ...
 */
export interface DataTableViewStateOld {
  columns: TableColumnsStateOld;
  sort: SortRuleOld[];
  filters: TableFilterOld[];
  query: string;
}

export interface TablePaginationOldProps extends Omit<ComponentPropsWithoutRef<'div'>, 'children'> {
