...
describe('TableSortMenuOld', () => {
  describe('the column list', () => {
    it('lists every column when nothing is sorted', async () => {
      render(<TableSortMenuOld columns={columns} rules={[]} {...handlers()} />);
      await open();
    ...
    it('starts sorting by a chosen column', async () => {
      const h = handlers();
      render(<TableSortMenuOld columns={columns} rules={[]} {...h} />);
      await open();
  ...
  describe('the active stack', () => {
    ...
    it('lists the active sorts in order', async () => {
      render(<TableSortMenuOld columns={columns} rules={sorted} {...handlers()} />);
      await open();
    ...
    it('reverses a direction', async () => {
      const h = handlers();
      render(<TableSortMenuOld columns={columns} rules={sorted} {...h} />);
      await open();
  ...
  describe('the trigger', () => {
    it('marks itself when something is sorted', () => {
      const { rerender } = render(
        <TableSortMenuOld columns={columns} rules={[]} {...handlers()} />
      );
...
describe('TableToolbarOld', () => {
  it('announces itself as one group rather than loose buttons', () => {
    ...
    const toolbar = screen.getByRole('toolbar', { name: 'TableOld controls' });
    expect(toolbar).toHaveAttribute('aria-orientation', 'horizontal');
  });

  it('takes a name of its own, since a page can hold more than one table', () => {
    render(<TableToolbarOld label="Ticket controls">x</TableToolbarOld>);
  ...
  it('pushes its actions to the trailing edge', () => {
    render(
      <TableToolbarOld>
        <span>lead</span>
        <TableToolbarActionsOld data-testid="actions">
          <span>trail</span>
  ...
  it('accepts extra classes', () => {
    render(
      <TableToolbarOld className="custom" data-testid="bar">
        x
      </TableToolbarOld>
    );
