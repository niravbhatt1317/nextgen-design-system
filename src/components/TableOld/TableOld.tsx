...
import { TableColumnBoundaryOld } from './TableColumnBoundary';
import {
  FROZEN_BAND,
  FROZEN_CELL,
  FROZEN_HEAD,
...
/**
 * Density and striping are set once on `TableOld` but applied by every cell, so
 ...
 * row announces its intent here and `TableCellOld` applies it.
 */
 ...
 */
export const tableHeadOldVariants = cva(
...
export const tableCellOldVariants = cva(
...
export const tableRowOldVariants = cva('mdt-transition-colors [&>*]:mdt-border-b', {
  variants: {
  ...
  compoundVariants: [
    ...
    // A summary row needs no equivalent: `TableRowOld` never passes `striped` for
 ...
 */
export const tableGroupRowOldVariants = cva(
...
const sortKey = (order: TableSortOrderOld): 'none' | 'ascend' | 'descend' => order ?? 'none';

/**
 * TableOld component - Root table element.
 *
 * @example
 * ```tsx
 * <TableOld density="condensed" striped stickyHeader>
 *   <TableHeaderOld>
 *     <TableRowOld>
 *       <TableHeadOld sortable sortOrder="ascend" onSort={sortByName}>Name</TableHeadOld>
 *       <TableHeadOld align="right">Amount</TableHeadOld>
 *     </TableRowOld>
 *   </TableHeaderOld>
 *   <TableBodyOld>
 *     <TableRowOld selected>
 *       <TableCellOld>Ada Lovelace</TableCellOld>
 *       <TableCellOld align="right">1,204.00</TableCellOld>
 *     </TableRowOld>
 *   </TableBodyOld>
 * </TableOld>
 * ```
 ...
 */
const TableOld = forwardRef<HTMLTableElement, TableOldProps>(
  (
    {
      className,
      children,
    ...
    return (
      <TableContext.Provider value={context}>
        <TableFrozenContext.Provider value={frozen}>
          ...
          >
            {/*
            TableOld is a compound component - headers are provided via TableHeaderOld/TableHeadOld children.
            Accessibility: Users must include <TableHeaderOld> with <TableHeadOld> cells for proper a11y.
          */}
            <table
              ref={ref}
              className={cn(
...
TableOld.displayName = 'TableOld';

/**
 * TableHeaderOld component - Contains table header rows.
 *
 * @example
 * ```tsx
 * <TableHeaderOld>
 ...
 * ```
 */
const TableHeaderOld = forwardRef<HTMLTableSectionElement, TableHeaderOldProps>(
  ({ className, ...props }, ref) => (
...
TableHeaderOld.displayName = 'TableHeaderOld';

/**
 * TableBodyOld component - Contains table data rows.
 *
 * @example
 * ```tsx
 * <TableBodyOld>
 ...
 * ```
 */
const TableBodyOld = forwardRef<HTMLTableSectionElement, TableBodyOldProps>(
  ({ className, ...props }, ref) => (
...
TableBodyOld.displayName = 'TableBodyOld';

/**
 * TableFooterOld component - Contains table footer rows, usually a total.
 *
 * @example
 * ```tsx
 * <TableFooterOld>
 *   <TableRowOld>
 *     <TableCellOld colSpan={3}>Total: $1,234.00</TableCellOld>
 *   </TableRowOld>
 * </TableFooterOld>
 * ```
 */
const TableFooterOld = forwardRef<HTMLTableSectionElement, TableFooterOldProps>(
  ({ className, ...props }, ref) => (
...
TableFooterOld.displayName = 'TableFooterOld';

/**
 * TableRowOld component - A table row.
 *
 * @example
 * ```tsx
 * <TableRowOld selected>
 *   <TableCellOld>Cell</TableCellOld>
 * </TableRowOld>
 * ```
 *
 * @deprecated Since 0.4.0. Use `Table` / `DataTable`, now the merged console
 ...
 * the removal pull request.
 */
const TableRowOld = forwardRef<HTMLTableRowElement, TableRowOldProps>(
...
TableRowOld.displayName = 'TableRowOld';

/**
 * TableGroupRowOld component - a heading row that spans the whole table.
 *
 ...
 * @example
 * ```tsx
 * <TableGroupRowOld colSpan={4} count={12} expanded={open} onToggle={toggle}>
 *   Mobile App
 * </TableGroupRowOld>
 * ```
 */
const TableGroupRowOld = forwardRef<HTMLTableRowElement, TableGroupRowOldProps>(
  (
    ...
    return (
      <tr ref={ref} className={cn(tableGroupRowOldVariants(), className)} {...props}>
        <td colSpan={colSpan} className={cn(tableCellOldVariants({ density }))}>
          {onToggle ? (
            <button
              type="button"
              onClick={onToggle}
...
TableGroupRowOld.displayName = 'TableGroupRowOld';

/**
 * TableExpandTriggerOld component - the disclosure control for a row that reveals
 * child rows beneath it.
 *
 * Deliberately a separate control you place in a cell rather than a prop on
 * `TableRowOld`. Where the chevron belongs differs from table to table, and the
 * component has no business owning your tree state.
 *
 * @example
 * ```tsx
 * <TableCellOld>
 *   <TableExpandTriggerOld expanded={open} onToggle={toggle} label="Show entries" />
 * </TableCellOld>
 * ```
 */
const TableExpandTriggerOld = forwardRef<HTMLButtonElement, TableExpandTriggerOldProps>(
...
TableExpandTriggerOld.displayName = 'TableExpandTriggerOld';

/**
 * TableHeadOld component - A table header cell, optionally a sort control.
 *
 * @example
 * ```tsx
 * <TableHeadOld>Name</TableHeadOld>
 * <TableHeadOld align="right">Amount</TableHeadOld>
 * <TableHeadOld sortable sortOrder="ascend" onSort={handleSort}>Created</TableHeadOld>
 * ```
 *
 * @deprecated Since 0.4.0. Use `Table` / `DataTable`, now the merged console
 * Users table. This earlier family stays only for side-by-side comparison until
 * the removal pull request.
 */
const TableHeadOld = forwardRef<HTMLTableCellElement, TableHeadOldProps>(
  (
    {
      className,
      align = 'left',
    ...
    return (
      <th
        ...
        className={cn(
          tableHeadOldVariants({ density, align }),
...
TableHeadOld.displayName = 'TableHeadOld';

/**
 * TableCellOld component - A table data cell.
 *
 * @example
 * ```tsx
 * <TableCellOld>Ada Lovelace</TableCellOld>
 ...
 * the removal pull request.
 */
const TableCellOld = forwardRef<HTMLTableCellElement, TableCellOldProps>(
  ({ className, align = 'left', indent = 0, frozen = false, columnKey, style, ...props }, ref) => {
    ...
    return (
      <td
        ...
        className={cn(
          tableCellOldVariants({ density, align, indent }),
...
TableCellOld.displayName = 'TableCellOld';

/**
 * TableCaptionOld component - A table caption/title.
 *
 * @example
 * ```tsx
 ...
 * ```
 */
const TableCaptionOld = forwardRef<HTMLTableCaptionElement, TableCaptionOldProps>(
  ({ className, ...props }, ref) => (
    <caption
      ref={ref}
...
TableCaptionOld.displayName = 'TableCaptionOld';
