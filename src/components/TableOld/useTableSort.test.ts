...
describe('useTableSortOld', () => {
  describe('starting state', () => {
    it('starts unsorted', () => {
      const { result } = renderHook(() => useTableSortOld<Key>());
    ...
    it('starts from what it was given', () => {
      const { result } = renderHook(() =>
        useTableSortOld<Key>({ initial: [{ column: 'status', direction: 'ascend' }] })
      );
    ...
    it('survives a re-render', () => {
      const { result, rerender } = renderHook(({ i }) => useTableSortOld<Key>({ initial: i }), {
  ...
  describe('more than one column', () => {
    ...
    it('replaces rather than stacks when only one sort is allowed', () => {
      const { result } = renderHook(() => useTableSortOld<Key>({ multiple: false }));
      act(() => {
        result.current.toggle('status');
      });
      act(() => {
