...
describe('TableViewMenuOld', () => {
  describe('the root panel', () => {
    it('shows what each panel currently says', async () => {
      render(<TableViewMenuOld columns={columns} {...handlers()} />);
      await open();
    ...
    it('names the column the table is grouped by', async () => {
      render(<TableViewMenuOld columns={columns} groupBy="status" {...handlers()} />);
      await open();
    ...
    it('marks the trigger once the view is not the default', () => {
      ...
      rerender(<TableViewMenuOld columns={columns} {...handlers()} />);
  ...
  describe('grouping', () => {
    it('groups by a column', async () => {
      const h = handlers();
      render(<TableViewMenuOld columns={columns} {...h} />);
      await open();
      await userEvent.click(screen.getByRole('button', { name: /^Group by/ }));
    ...
    it('clears grouping through None', async () => {
      const h = handlers();
      render(<TableViewMenuOld columns={columns} groupBy="status" {...h} />);
      await open();
      await userEvent.click(screen.getByRole('button', { name: /^Group by/ }));
  ...
  describe('columns', () => {
    ...
    it('offers Show all and Hide all only when given handlers', async () => {
      const h = handlers();
      const { rerender } = render(<TableViewMenuOld columns={columns} {...h} />);
      await open();
      await userEvent.click(screen.getByRole('button', { name: /^Columns/ }));
      expect(screen.queryByRole('button', { name: 'Show all' })).not.toBeInTheDocument();

      const onShowAll = vi.fn();
      rerender(<TableViewMenuOld columns={columns} onShowAll={onShowAll} {...h} />);
    ...
    it('calls Hide all', async () => {
      ...
      render(<TableViewMenuOld columns={columns} onHideAll={onHideAll} {...handlers()} />);
      await open();
      await userEvent.click(screen.getByRole('button', { name: /^Columns/ }));
