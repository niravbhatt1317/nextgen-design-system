 ...
 */
export interface EditableTabOld2 {
  /** Stable and unique. Not the label - a renamed tab is still the same tab. */
  id: string;
  label: string;
}

export interface UseEditableTabsOld2Options {
  /** What the set starts as. */
  initialTabs?: EditableTabOld2[];
...
export interface UseEditableTabsOld2 {
  tabs: EditableTabOld2[];
  active: string;
 ...
 *   it.
 */
export const useEditableTabsOld2 = ({
  initialTabs = [{ id: nextId(), label: 'Tab 1' }],
  initialActive,
  newTabLabel = (index) => `Tab ${index.toString()}`,
  allowEmpty = false,
}: UseEditableTabsOld2Options = {}): UseEditableTabsOld2 => {
  const [tabs, setTabs] = useState<EditableTabOld2[]>(initialTabs);
